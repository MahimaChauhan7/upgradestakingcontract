...
contract StakingContract {
    ...
    mapping(address => uint256) lastTimeupdate;
    ...
    function getRewards(address user) public view returns (uint256) {
        uint256 updateTime = lastTimeupdate[user];
        uint256 currentReward = unclaimedReward[user];
        // calculate the reward when lastTimeupdate means, when claimed or stake or unstake
        uint256 newReward = (block.timestamp - updateTime) * balanceOf(user);
